...
const enabled = String(process.env.SERVER_COLLECTOR_ENABLED || 'false').toLowerCase() === 'true';
...
const userAgent = process.env.HTTP_USER_AGENT || 'BlazeAnalyticsCollector/1.1';
...
async function collectCurrent() {
  try {
    ...
    await saveSnapshot(current, exposure, 'server');
...
async function collectRecent() {
  try {
    ...
    for (const round of recent) {
      ...
      if (round?.id != null && color) await finalizeRound(round, color, 'server');
    }
...
async function startCollector() {
  if (!enabled) {
    console.log('Coletor direto do servidor desativado; aguardando dados enviados pela extensão.');
    return;
  }
  if (running) return;
  running = true;
  ...
  console.log(`Coletor do servidor iniciado: current=${currentPollMs}ms recent=${recentPollMs}ms`);
}
...
function collectorStatus() {
  return { enabled, running, mode: enabled ? 'server' : 'extension-ingest', baseUrl, currentPollMs, recentPollMs, lastCurrentError, lastRecentError };
}
